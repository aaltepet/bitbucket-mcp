...
const SENSITIVE_KEY_PATTERN = /^(authorization|proxy-authorization|password|passwd|token|access_token|refresh_token|api[_-]?key|secret|client_secret|BITBUCKET_TOKEN|BITBUCKET_PASSWORD)$/i;
 ...
 */
export function escapeBitbucketQueryValue(value) {
    return value.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
}
/**
 * Build a Bitbucket repository name filter that cannot break out of the quoted value.
 */
export function buildRepositoryNameFilter(name) {
    ...
    if (isPlainObject(value)) {
        ...
        for (const [key, nested] of Object.entries(value)) {
            ...
            if (nested &&
                ...
                (typeof nested.pipe === "function" ||
                    typeof nested._header === "string")) {
                result[key] = "[Omitted]";
                continue;
            }
...
export const redactSecretsFormat = winston.format((info) => {
    const sanitized = sanitizeForLog(info);
 ...
 */
export function isAllowedNextUrl(nextUrl, options) {
    let configuredBase;
    try {
...
export function assertAllowedNextUrl(nextUrl, options) {
    if (!isAllowedNextUrl(nextUrl, options)) {
        throw new Error(`Refusing to follow pagination next URL outside allowlisted Bitbucket API origin: ${nextUrl}`);
    }
}
